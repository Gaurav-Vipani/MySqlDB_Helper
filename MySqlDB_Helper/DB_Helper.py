...
class DB_Helper:
    ...
    def insert_records(self, table_name="", column_name=list(), column_values=tuple(), multiple_record=False):
        """
    ...
    def fetch_records(self, table_name="", column_name=list("*"), where_key=list(), where_value=list(), where_clause=False):
        """
        ...
        Parameter :
            ...
            4) where_key : provide column name used for where condition and type is list.
                           Default values is Empty list.

            5) where_value : provide column value used for fetch specific value based on condition record and type is list.
                             Default values is Empty list.

            6) where_clause : it  is Boolean Value. for fetch records using where clause.
        ...
        Ex. -
            ...
            fetch_records("user",["name","phone"],["id"],["16"], where_clause=True)
        """
        data = dict()
        data['message'] = None
        data['status'] = None
            ...
            if table_name != "":
                ...
                if column_name != []:
                    ...
                    if where_clause != True:
                        ...
                        where_key_val = [f"{where_key[i]}={where_value[i]}" for i in range(len(where_key))]
                        query = f"SELECT {', '.join(column_name)} FROM {table_name} WHERE {' and '.join(where_key_val)}"
                        print(query)
    ...
    def update_record(self, table_name="", column_name=list(), column_value=list(), where_key=list(), where_value=list()):
        """
        ...
        Parameters :
            ...
            5) where_value : provide column value used for update specific value record and type is list.
                             Default values is Empty list.
        ...
        Ex. -
            -> single values update with single condition.
            db_helper.update_record("user", ["name"], ["Gaurav"], ["id"], ["6"])

            -> single values update with multiple condition.
            db_helper.update_record("user", ["name"], ["Gaurav"], ["id", "phone"], ["6", "9033930098"])

            -> multiple column values update with single condition.
            db_helper.update_record("user", ["name","phone"], ["Gaurav", "910123456789"], ["id"], ["6"])

            -> multiple column values update with multiple condition.
            db_helper.update_record("user", ["name",""gender], ["Gaurav","M"], ["id", "phone"], ["6", "9033930098"])
        ...
        try:
            ...
            if table_name != "":
                ...
                if column_name != [] and column_value != [] and where_key != [] and where_value != []:

                    column_key_val = [f"{column_name[i]}='{column_value[i]}'" for i in range(len(column_name))]
                    where_key_val = [f"{where_key[i]}='{where_value[i]}'" for i in range(len(where_key))]
                    query = f"UPDATE {table_name} SET {','.join(column_key_val)} WHERE {' and '.join(where_key_val)}"
                    cursor.execute(query)
                ...
                elif (not column_name) or (not column_value) or (not where_key) or (not where_value):
    ...
    def delete_record(self, table_name="", where_key=list(), where_value=tuple()):
        """
        ...
        Parameters :
            ...
            2) where_key = provide column_name based on our condition and value type is list.
                           Default values = list()
            3) where_value = provide column_name based on our condition and value type is list.
                             Default values = list()
        Ex. -
            -> for deleting Record Single Condition
               db_helper.delete_record("user", "name", "Divyesh")

            -> for deleting Records Multiple Condition
               db_helper.delete_record("user", ["name","city"], [("Dhruv","Rajkot"),("Divyesh", "Rajkot")])

            -> for deleting Records single condition Multiple Records
               db_helper.delete_record("user", "city", [("Rajkot"), ("Ahmedabad")])
        ...
        try:
            ...
            if table_name != "":
                ...
                if where_key != [] and where_value != []:
                    ...
                    where_key_val = [f"{where_key[i]}= '{where_value[i]}'" for i in range(len(where_key))]
                    query = f"DELETE FROM {table_name} WHERE {' and '.join(where_key_val)}"
                    cursor.execute(query)
